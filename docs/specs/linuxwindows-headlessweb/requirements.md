...
为当前 Linux / Windows Headless/Web 项目增加可持续验证的 GitHub Actions 自动发布能力。普通 `main` push / PR 自动执行 CI；影响发布构建的 `main` push 自动执行三平台 release build 验证但不发布；推送 `v*` tag 时才创建 GitHub Release。手动 dispatch 可以针对任意 ref 做三平台 build-only 验证，只有显式选择 publish 且 ref 为已有 `v*` tag 时才允许发布。
...
- Release workflow 在相关 `main` push 上执行 verify + 三平台 build，但 publish job 仅对 `v*` tag 或显式手动 publish 开启。
...
2. WHEN 相关代码或 Release workflow 被 push 到 `main` THEN Release workflow SHALL 执行 verify 与三平台 build 且 SHALL NOT 创建 Release。
3. WHEN 维护者手动 dispatch 且 `publish=false` THEN workflow SHALL 对指定 ref 执行 verify 与三平台 build 且 SHALL NOT 创建 Release。
...
- **NFR-4（Fork 前置条件）**: 仓库是 GitHub fork 时，Actions 必须先在该 fork 的 Actions 页面启用；此设置属于 GitHub 仓库级状态，不由 workflow YAML 自动修改。
