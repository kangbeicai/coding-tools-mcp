...
export async function getWorkspaceSecret(
  ...
  return invokeCommand<string | null>("get_workspace_secret", { id, key });
}
...
export async function setWorkspaceSecret(
  ...
  return invokeCommand<void>("set_workspace_secret", { id, key, value });
}
...
export async function regenerateWorkspaceSecret(
  ...
  return invokeCommand<string>("regenerate_workspace_secret", { id, key });
}
...
export async function setSharedSecret(key: SharedSecretKey, value: string): Promise<void> {
  return invokeCommand<void>("set_shared_secret", { key, value });
}

export async function regenerateSharedSecret(key: SharedSecretKey): Promise<string> {
  return invokeCommand<string>("regenerate_shared_secret", { key });
}
